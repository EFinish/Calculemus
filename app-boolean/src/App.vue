...
// A ?u={id} query is a shared universe: load it read-only. (Query, not a
// path route, so this edition works under any mount prefix.)
onMounted(async () => {
  const id = new URLSearchParams(location.search).get("u");
  if (id && /^[a-z0-9]{10}$/.test(id)) importError.value = await loadShared(id);
});
...
<template>
  <header>
    <span class="wordmark">∴ Calculemus <span class="edition">Boolean</span></span>
    <a class="edition-link" href="/" title="The newer edition, with relational statements (M6)">relational edition →</a>
